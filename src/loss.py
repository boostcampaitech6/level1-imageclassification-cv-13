...
class CutMixF1Loss(F1Loss):
    def __init__(self, classes=3, epsilon=1e-7):
        super().__init__(classes, epsilon)

    def forward(self, y_pred, y_mixed):
        labels, shuffled_labels, lam = y_mixed
        loss = lam * self(y_pred, labels) + (1 - lam) * self(y_pred, shuffled_labels)
        return loss
