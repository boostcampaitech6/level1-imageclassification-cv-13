...
from torchvision.transforms.transforms import RandomAffine, RandomHorizontalFlip
...
wandb.init(project="project1", entity="guseob")
...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        # precision = precision_score(labels_list, preds_list, average='macro')  # Calculate precision
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                # labels는 레이블의 리스트입니다.
                wandb.log({"label_distribution": wandb.Histogram(labels.cpu().numpy())})
        ...
        with torch.no_grad():  # 그래디언트 계산 비활성화
            predictions = []
            true_labels = []
            for inputs, labels in val_loader:  # 데이터 로더에서 배치 가져오기
                inputs = inputs.to(device)
                labels = labels.to(device)
                outs = model(inputs)  # 모델의 출력 계산
                preds = torch.argmax(outs, dim=-1)  # 가장 높은 확률을 가진 클래스 선택
        ...
        f1 = f1_score(true_labels, predictions, average='macro')  # 'macro': 클래스 불균형을 고려하지 않음
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", type=int, default=2021, help="random seed (default: 42)"
    )
    parser.add_argument(
        "--epochs", type=int, default=30, help="number of epochs to train (default: 1)"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--optimizer", type=str, default="Lion", help="optimizer type (default: SGD)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pretrained", action = "store_true", help="pretrained model True or False"
    )
