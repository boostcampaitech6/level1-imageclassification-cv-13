...
class F1_Cross_focal__comb_Loss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        cross = self.cross(y_pred, y_true)
        focal = self.focal(y_pred, y_true)
        smoothing = self.smoothing(y_pred, y_true)
        return 0.3 * f1 + 0.3 * smoothing + 0.2 * focal + cross * 0.3
class F1_CrossEntropy_comb_Loss(nn.Module):
    def __init__(self, alph=0.5):
        super().__init__()
        self.f1 = F1Loss()
        self.cross = nn.CrossEntropyLoss()
    ...
    def forward(self, y_pred, y_true):
        ...
        cross = self.cross(y_pred, y_true)
        focal = self.focal(y_pred, y_true)
        return 0.2 * f1 + 0.3 * cross + 0.5 * focal
...
_criterion_entrypoints = {
    ...
    "f1": F1Loss,
    "comb": F1_CrossEntropy_comb_Loss,
    "comb2": F1_Cross_focal__comb_Loss,
}
...
def create_criterion(criterion_name, alph, **kwargs):
    """
